---
name: storytelling-viz-python
description: Story-first Seaborn/Matplotlib workflow for turning a dataset into a polished static visualization with local review artifacts. Use this skill when the user wants a concise, intuitive chart with a clear takeaway, plus final `viz.png` and `preview.html`, without website publishing work.
---

# Storytelling Viz (Python)

Use this skill when the task is only about choosing, building, and refining the visualization itself using Seaborn and Matplotlib.

## Portability Note

`SKILL.md` is the canonical cross-tool instruction file for this skill.

- Keep the core workflow and output contract here.
- Keep tool-specific metadata outside this file.

The goal is to produce one Seaborn/Matplotlib visualization that is:
...
- `viz.png` (or `viz.svg`)
- `preview.html` (wrapper to view the image)
- 2-3 insight bullets

Do not take on website publishing. If the user wants the visualization inserted into a site or blog post, use a separate publishing skill.
...
- comprehension without interaction (static)
...
- wants an embeddable static visualization in Seaborn/Matplotlib
...
6. Build the first draft in Seaborn/Matplotlib.
...
## Seaborn and Matplotlib Rules

Use Seaborn by default for high-level statistical plotting. Use Matplotlib directly for layout control and custom annotations.

Prioritize:

- clear ranking or comparison
- readable labels and high DPI (e.g., 300)
- restrained color
- titles and subtitles that carry the narrative
- stable layouts with fixed aspect ratios
- direct labels or annotations instead of legends when possible
- one label system per mark when possible; avoid stacking bar-end labels and callout text
- concise footers for source information

Avoid:

- excessive traces or overlapping marks
- decorative marker layers that make endpoints feel ambiguous
- default library aesthetics left unrefined
- dense plots with no visual hierarchy
- pie charts or donut charts

For `preview.html` wrappers that embed the visualization:
...
- prefer a split of roles: `preview.html` carries the main takeaway and the visualization file carries a shorter functional title plus a concise orienting subtitle
- if the same headline appears both in the wrapper and inside the image, compress one of them until the duplication is gone
- inside the image frame, avoid oversized hero blocks; let the chart area take priority quickly
...
- the styling must support one clear takeaway
...
Load [references/seaborn-matplotlib-patterns.md](references/seaborn-matplotlib-patterns.md) when building the visualization. Use those patterns for:

- layout defaults
- figure sizing and DPI
- annotations
- responsive embed wrappers (for preview.html)
- export structure
...
- review the final `viz.png` or `preview.html` in a rendered state
...
- the final visualization artifact (`viz.png` or `viz.svg`)
...
- `viz/project-name/viz.png` for the visualization
